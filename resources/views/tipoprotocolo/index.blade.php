...
<div class="container">
    ...
    <div class="row">
        <div class="col-12 mx-auto">
            <h4 class="text-white"><strong class="text-capitalize">{{__('asignación de protocolos')}}</h4>
            {{-- @include('partials.error') --}}
            @can('tipoprotocolos.store')
            <form class="bg-white shadow rounded py-3 px-3" action="{{route('tipoprotocolos.store',$tipo->id)}} "
                method="POST">
                @csrf
                @method('post')
                <div class="row">
                    ...
                    <div class="col-6">
                        <label>
                            <input type="checkbox" id="id" value="{{$p->id}}" class="mx-2 t" name="id[]"
                                @if($protocolosId->contains($p->id))) checked @endif>
                            <small>{{$p->tipo}} :</small>
                            {{$p->tarea}}
                            <hr>
                        </label>
