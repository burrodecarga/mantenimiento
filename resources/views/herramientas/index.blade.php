        ...
        <div class="col-md-12 mx-auto">
            <div class="card">
                <div class="card-header">
                    <h5 class="display-6 text-center mt-2 p-0 text-uppercase ">{{__('tools list')}} </h5>
                </div>
                <div class="card-body">
                    ...
                    <div class="col  d-flex justify-content-between  align-items-center">
                        <span>
                            ...
                            <a href="{{route('herramientas.create')}} " class="btn btn-primary my-2 float-left"
                                title="{{__('add register')}} "><i class="fa fa-plus-circle" aria-hidden="true"></i>
                                {{__('new record')}} </a>
                            @endcan
                        </span>
                        <span>
                            @can('herramientas.formImportExcel')
                            <a href="{{route('herramientas.formImportExcel')}}" class="btn btn-primary"><i
                    ...
                    <table aria-colspan="2" id="example" class="table table-striped table-bordered" style="width:100%">
                        <thead>
                            <tr>
                                <th width="15%" class="text-capitalize">{{__('herramienta')}}</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td scope="row" class="text-capitalize">{{$herramienta->name}}</td>
                                <td scope="row" class="text-capitalize text-right">
                                    {{number_format($herramienta->precio, 2)}}</td>
                                <td scope="row" class="text-capitalize text-right">
                                    {{number_format($herramienta->existencia,2)}}</td>
                                <td scope="row" class="text-capitalize">{{$herramienta->proveedor}}</td>
                                <td scope="row" class="text-capitalize">{{$herramienta->description}} </td>
                                <td class="text-center">
                                    ...
                                    document.getElementById({{$herramienta->id}}).submit();" data-toggle="tooltip"
