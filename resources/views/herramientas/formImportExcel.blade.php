...
<div class="container">
  ...
  <div class="card-body">
    ...
    <h5>Nombre del archivo: herramientas.xlxs</h5>
      <form action="{{route('herramientas.importExcel')}} " method="POST" enctype="multipart/form-data">
           @method('POST');
            @csrf
...
<div class="card text-left">
    ...
    <div class="card-body">
      ...
      <p class="card-text">Elimina todos los registros de herramientas</p>
        <form action="{{route('herramientas.vaciar')}} " method="POST" enctype="multipart/form-data">
             @method('POST');
              @csrf
              @if(Session::has('message'))
              <div class="alert alert-success" role="alert">
