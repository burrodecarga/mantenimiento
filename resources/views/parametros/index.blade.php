                ...
                <div class="card-body">
                    ...
                    <div class="col  d-flex justify-content-between  align-items-center">
                        <span>
                            ...
                            <a href="{{route('parametros.create')}} " class="btn btn-primary my-2 float-left"
                            title="{{__('add register')}} "><i class="fa fa-plus-circle" aria-hidden="true"></i>
                            {{__('new record')}} </a>
                            @endcan
                        </span>
                        <span>
                            @can('parametros.formImportExcel')
                                <a href="{{route('parametros.formImportExcel')}}" class="btn btn-primary"><i class="fa fa-download" aria-hidden="true"></i> </a>
                        @endcan
                                        </span>
                    ...
                    <table aria-colspan="2" id="example" class="table table-striped table-bordered" style="width:100%">
                        <thead>
                            <tr>
                               <th width="30%" class="text-capitalize">{{__('parameter')}}</th>
                               <th width="10%" class="text-capitalize">{{__('valor')}}</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td scope="row" class="text-capitalize">{{$parametro->name}} </td>
                                <td scope="row" class="text-capitalize">{{$parametro->valor}} </td>
                                <td scope="row" class="text-capitalize">{{$parametro->unidad}} </td>
                                <td scope="row" class="text-capitalize">{{$parametro->description}} </td>
                                <td class="text-center">
                                   ...
                                   @can('parametros.edit')
                                    ...
                                    document.getElementById({{$parametro->id}}).submit();" data-toggle="tooltip"
                                        data-placement="top" title="{{__('delete record')}} ">
                                        <i class="fa fa-trash" aria-hidden="true"></i>
                                    </a>
                                    <form action="{{route('parametros.destroy',[$parametro->id])}} " method="post"  id="{{$parametro->id}}">
                                    @csrf
                                    @method('delete')
                                    </form>
                                    @endcan
